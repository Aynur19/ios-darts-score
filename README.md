...
- ссылка на сайт с описанием: **[Darts Score App](https://aynur19.github.io/ios-darts-score/)**
...
| 2| Применение стандартных компонентов `SwiftUI`: `View`, `TabView`, `[X]Stack`, `Button`, `ScrollView`, `GeometryReader`, `NavigationStack` и т.д.|
| 3| Разработка приложения по архитектуре **MVVM**|
| 4| Использование свойств и модификаторов `SwiftUI`, таких как, `@Environment`, `@EnvironmentObject`, `@State`, `@StateObject`, `@ObservedObject` и `@Published` для управления данными и их обновлением в приложении|
| 5| Использование `Combine`|
| 6| Реализация навигации и переходов в приложении|
| 7| Сериализация игровых данных в **JSON** с применением протокола `Codable`|
| 8| Сохранение пользовательских настроек с использованием `UserDefaults` и `@AppStorage`|
| 9| Подключение и использование через **SPM** сторонних библиотек, например, **SwiftUIComponents4iOS**|
|10| Реализация кастомных интерактивных компонентов `VGradientView`, `HWheelPickerView`, `HStepperView`, `HStaticSegmentedPickerView`, `BluredPopup` и использовние через пакет `SwiftUIComponents4iOS`|
|11| Реализация для стандартных UI-компонентов собственных стилей: `ImageToggleStyle` и `PrimaryButtonStyle`|
|12| Применение асинхронных вызовов функций (**Swift Concurrency**) для повышения отзывчивости приложения|
|13| Использование **SwiftLint** для проверки кода на наличие ошибок и поддерживания code-style в проекте|
|14| Использование условия `#available` для проверки доступности API на определенной версии операционной системы и написания разной реализации в зависимости от выполнения условия|
|15| Польностью программное построение и отрисовка мишени для дартса из примитивов `Path` и `Text`|
|16| Работа с изображениями и звуком (`AVFoundation`)|
|17| Работа с анимациями и анимированными переходами|
|18| Добавление локализации приложения (на русский язык)|
|19| Оптимизация производительности приложения|
|20| Обработка исключений|
|21| Расширение функционала стандартных структур и классов через `extension`|